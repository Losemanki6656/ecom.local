            ...
            <div class="card">
                <div class="card-body p-0">
                    ...
                    <form class="p-4" action="{{ route('cities.update', $city->id) }}" method="POST"
                        ...
                        <div class="form-group mb-3">
                            <label for="name">{{ translate('Emu Town') }}</label>
                            <select class="form-control aiz-selectpicker rounded-0" name="adress_emu"
                                data-live-search="true" required>
                                <option value="">{{ translate('Select Shop Address EMU') }}</option>
                                @foreach ($townLists as $town)
                                    <option value="{{ $town['name'] }}" @if ($city->emu_town == $town['name']) selected @endif
