...
@section('panel_content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form class="" action="{{ route('seller.shop.update') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row mb-3">
                    <label class="col-md-2 col-form-label">{{ translate('Shop Address EMU') }} <span
                            class="text-danger text-danger">*</span></label>
                    <div class="col-md-10">
                        <select class="form-control aiz-selectpicker rounded-0" name="adress_emu" data-live-search="true"
                            required>
                            <option value="">{{ translate('Select Shop Address EMU') }}</option>
                            @foreach ($townLists as $town)
                                <option value="{{ $town['name'] }}" @if ($shop->emu_town == $town['name']) selected @endif
                                    data-content="<span class='d-block'>
                                    <span class='d-block fs-16 fw-600 mb-2'> {{ $town['name'] }}</span>
                                    <span class='d-block opacity-50 fs-12'><i class='las la-map-marker'></i> {{ $town['city']['name'] }}</span>
                                    </span>">
                                </option>
                            @endforeach
                        </select>
    ...
    @if ($shop->billz_status)
        <div class="card">
            <div class="card-header">
                <h5 class="mb-0 h6">{{ translate('Billz Info') }}</h5>
            </div>
            <div class="card-body">
                <form class="" action="{{ route('seller.shop.update') }}" method="POST">
                    <input type="hidden" name="shop_id" value="{{ $shop->id }}">
                    ...
                    <div class="form-box-content p-3">
                        <div class="row mb-3">
                            <label class="col-md-2 col-form-label">{{ translate('Sub name') }}</label>
                            <div class="col-md-10">
                                <input type="text" class="form-control" name="key" value="{{ $shop->key }}">
                            </div>
                        </div>
                        <div class="row mb-3">
                            <label class="col-md-2 col-form-label">{{ translate('Secret') }}</label>
                            <div class="col-md-10">
                                <textarea type="text" class="form-control" name="secret">{{ $shop->secret }}</textarea>
                            </div>
                        </div>
